<?php
/**
 * Description of AP_ContactsWorkersEdit
 *
 * @author olga
 */
class AP_ContactsWorkersEdit extends AdminPanel_ComponentPanelUI_Element_Edit {
    
    private $workersLangData; 
    private $listWorkersData; 
    ...
    protected function getData() {
        ...
        $this->data = array();
        $query = "SELECT * FROM `ContactsWorkers` WHERE `id`='".$this->editElement."';";
        $this->data = $this->SQL_HELPER->select($query,1);
        $query = "SELECT * FROM `ContactsWorkers_Lang` WHERE `worker`='".$this->editElement."';";
        $this->workersLangData = $this->SQL_HELPER->select($query);
        $query = "SELECT `list` FROM `ContactsWorkers_has_ContactsWorkersList` WHERE `contact`='".$this->editElement."';";
        $this->listWorkersData = $this->SQL_HELPER->select($query,1);
    } 
    ...
    protected function setDefaltInput() {
        ...
        foreach ($this->workersLangData as $langData) {
            $this->insertValue['fio'][$langData['lang']] = $langData['fio'];
            $this->insertValue['info'][$langData['lang']] = $langData['info'];
        }
        if ($this->listWorkersData < 0 || $this->listWorkersData == null) {
            $this->insertValue['list'] = "0";
        } else {
            $this->insertValue['list'] = $this->listWorkersData['list'];
        }
    ...
    protected function updateExecute() {
        ...
        $queryContactsWorkers .= "`post`=".InputValueHelper::mayByNull($this->insertValue['post']).", ";
        ...
        $queryContactsWorkers .= " WHERE `id`='".$this->editElement."';";
        ...
        $queryContactsWorkers_Lang_LangDel = "DELETE FROM  `ContactsWorkers_Lang` WHERE  `worker` =  '".$this->editElement."';";
        $this->SQL_HELPER->insert($queryContactsWorkers_Lang_LangDel);
    ...
    protected function checkEditElement() {
        $query = "SELECT * FROM `ContactsWorkers` WHERE `id`= '".urldecode($this->editElement)."';";
