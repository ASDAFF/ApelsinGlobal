...
class ContactsShop {
    ...
    private function getData() {
        ...
        if($this->showReviews) {
            global $_URL_PARAMS;
            $page = isset($_URL_PARAMS['params'][0]) ? $_URL_PARAMS['params'][0] : null;
            $this->reviews = new Reviews($page);
        }
    ...
    private function generateHTML() {
        ...
        if($this->showReviews) {
            $this->html .= "<div class='reviews'>";
            $this->html .= $this->reviews->getHTML();
            $this->html .= "</div>";
        }
        $this->html .= "<hr>";
    ...
    private function generatePrintHTML() {
        ...
        if($this->showReviews) {
            $this->printHtml .= "<span class='checkbox_text'>";
            $this->printHtml .= '<input type="checkbox" id="checkbox_shopIDreviews" checked onClick="change_show_hide()">';
            $this->printHtml .= $this->localization->getText('shopIDreviews_text');
            $this->printHtml .= "</span>";
        }
        ...
        #reviews
        if($this->showReviews) {
            $this->printHtml .= "<div class='reviews' id='shopIDreviews' style='display: ;'>";
            $this->printHtml .= $this->reviews->getPrintHTML();
            $this->printHtml .= "</div>";
            $this->printHtml .= "<div class='clear'></div>";
        }
